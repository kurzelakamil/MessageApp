			...
			<div class="pull-right" style="margin-top: 20px;">
				<ul>
					<li style="display: inline"><a
						href="/MessageApp/MessageController" role="button"
						class="btn btn-primary" name="message">Messages<span
		...
		<div class="new-message">
			<form action="/MessageApp/Homepage" method="POST">
				<div class="form-group">
					...
					<textarea class="form-control" id="textSubject" rows=1
						name="subject"></textarea>
					<label for="textMessage">Message</label>
					<textarea class="form-control" id="textMessage" rows=15
						name="newMessage"></textarea>
				</div>
				<div class="form-group pull-left">
					<script>
						$('#textMessage').on('keyup', function() {
							$(this).next().text('Text length: ' + $(this).val().length);
				...
				<div class="form-group pull-right">
					<label for="receiver">Recipient</label> <select
						multiple-class="form-control" id="receiver" name="receiver"
						class="custom-select custom-select-sm">
						<c:forEach items="${sessionScope.usernames}" var="item">
							<option>${item}</option>
						</c:forEach>
					</select> <input type="submit" class="btn btn-primary" value="Send">
				</div>
			</form>
		</div>
	</div>
